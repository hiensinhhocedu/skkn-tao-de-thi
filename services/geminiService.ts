...
import type { Objective, ExamData, MatrixData, SpecificationData, MatricesData } from '../types';
...
`;
    ...
    if (config.generationMethod === 'topic') {
        ...
        prompt += `- Dựa trên Yêu cầu cần đạt (YCCĐ) - ĐÂY LÀ CĂN CỨ QUAN TRỌNG NHẤT:\n`;
        config.objectives.forEach((obj: Objective, index: number) => {
            prompt += `  ${index + 1}. Chủ đề: ${obj.topic}, YCCĐ: ${obj.requirement}\n`;
        });
        prompt += `\nLƯU Ý: Bạn PHẢI bám sát các YCCĐ trên để xây dựng ma trận, đặc tả và câu hỏi. Không được tự ý thêm các nội dung ngoài danh sách YCCĐ này.\n`;
    }
...
3. Đối với phương pháp "Dựa trên YCCĐ": Mỗi hàng trong \`topicRows\` PHẢI tương ứng với một Chủ đề/YCCĐ đã được cung cấp. Tên chủ đề trong ma trận phải phản ánh đúng nội dung YCCĐ đó.
4. Câu hỏi "Đúng/Sai":
...
2. **ĐỐI VỚI PHƯƠNG PHÁP YCCĐ**: Nội dung trong các trường này PHẢI được trích xuất hoặc cụ thể hóa trực tiếp từ danh sách YCCĐ mà người dùng đã cung cấp ở trên. Bạn đóng vai trò là chuyên gia sư phạm để phân rã YCCĐ đó thành các mức độ Biết, Hiểu, Vận dụng tương ứng.
3. Nội dung của mỗi trường phải là một mô tả **chi tiết và cụ thể** về Yêu cầu cần đạt tương ứng với mức độ nhận thức đó (Biết, Hiểu, Vận dụng) và phù hợp với nội dung của chủ đề.
...
1. **BẮT BUỘC ĐỒNG BỘ VỚI MA TRẬN ĐẶC TẢ VÀ YCCĐ**: 
   - Bạn phải tạo đủ câu hỏi cho TẤT CẢ các phần có số lượng > 0 trong ma trận đặc tả.
   - **ĐỐI VỚI PHƯƠNG PHÁP YCCĐ**: Mỗi câu hỏi được tạo ra PHẢI nhằm mục đích kiểm tra chính xác Yêu cầu cần đạt (YCCĐ) tương ứng trong ma trận đặc tả. Nội dung câu hỏi không được lệch khỏi phạm vi của YCCĐ đã cho.
